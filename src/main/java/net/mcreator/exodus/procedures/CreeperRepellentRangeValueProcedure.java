...
public class CreeperRepellentRangeValueProcedure {
	public static double execute(LevelAccessor world, ItemStack itemstack) {
		if ((itemstack.getEnchantmentLevel(world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ResourceKey.create(Registries.ENCHANTMENT, Identifier.parse("exodus:mystical_conductivity")))) != 0) == true) {
			return itemstack.getEnchantmentLevel(world.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ResourceKey.create(Registries.ENCHANTMENT, Identifier.parse("exodus:mystical_conductivity"))));
		}
		return 0;
	}
}