...
public class GreedLevelGoldProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, BlockState blockstate, Entity entity) {
		...
		if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel == 0) {
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldGoal = 20;
				_vars.markSyncDirty();
			}
		} else if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel == 1) {
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldGoal = 50;
				_vars.markSyncDirty();
			}
		} else if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel == 2) {
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldGoal = 100;
				_vars.markSyncDirty();
			}
		} else if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel == 3) {
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldGoal = 150;
				_vars.markSyncDirty();
			}
		} else if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel == 4) {
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldGoal = 200;
				_vars.markSyncDirty();
		...
		if (blockstate.is(BlockTags.create(Identifier.parse("the_better_server:gold_blocks"))) == true) {
			...
			if ((entity instanceof LivingEntity _livEnt12 && _livEnt12.hasEffect(TheBetterServerModMobEffects.GOLD_RUSH)) == true) {
			...
			{
				TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
				_vars.goldAmount = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).goldAmount + (goldAmount + gold_extra + gold_rush_additive) * gold_rush_multiplier;
				_vars.markSyncDirty();
			}
			if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).goldAmount >= entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).goldGoal) {
				{
					TheBetterServerModVariables.PlayerVariables _vars = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES);
					_vars.goldAmount = 0;
					_vars.greedLevel = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel + 1;
					_vars.markSyncDirty();
				}
