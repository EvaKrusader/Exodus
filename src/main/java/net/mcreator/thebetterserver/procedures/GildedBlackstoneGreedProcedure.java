...
public class GildedBlackstoneGreedProcedure {
	...
	private static void execute(@Nullable Event event, BlockState blockstate, Entity entity) {
		...
		if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel != 0) {
			startChance = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel * 0.19;
			durationSeconds = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel * 7.4 + 3.6;
			power = entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).greedLevel * 0.65 + 0;
		}
		if (blockstate.getBlock() == Blocks.GILDED_BLACKSTONE && startChance >= Math.random()) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
				_entity.addEffect(new MobEffectInstance(TheBetterServerModMobEffects.GOLD_RUSH, (int) (durationSeconds * 20), (int) power, false, false));
		}
	}
}