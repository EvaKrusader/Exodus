...
public class DeathContractDiesProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).hasDeathContract == true) {
			...
			if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1) <= 6) {
				if (entity.getData(TheBetterServerModVariables.PLAYER_VARIABLES).playContractSound == false) {
